...
logger = logging.getLogger("shadow_mm_ws")
logger.setLevel(logging.INFO)
if not logger.handlers:
    handler = logging.StreamHandler()
    handler.setFormatter(logging.Formatter("[SHADOW_MM_WS] %(asctime)s %(message)s"))
    ...
    while True:
        try:
            async with websockets.connect(ws_url, additional_headers=headers) as websocket:
                ...
                metrics.last_connect_ts = time.monotonic()
                backoff = 1.0
                ...
                async for raw in websocket:
                    metrics.msg_count += 1
                    metrics.last_message_ts = time.monotonic()
                    data = json.loads(raw)
