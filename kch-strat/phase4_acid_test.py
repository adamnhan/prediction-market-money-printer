#!/usr/bin/env python3
"""Phase 4 acid test: compute market-level PnL/diagnostics after removing a price band."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 4 acid test: remove band and recompute PnL/diagnostics")
    ...
    parser.add_argument("--remove_band", default="30,40")
    parser.add_argument("--infer_winner_threshold", type=float, default=95.0)
    ...
    # Remove 30-40 band trades
    band_lo, band_hi = [float(x) for x in args.remove_band.split(",")]
    trades = trades[~trades["price_cents_norm"].between(band_lo, band_hi, inclusive="both")].copy()
    log_fn(f"removed band {band_lo}-{band_hi} rows now={len(trades)}")
    ...
    for market_key, group in trades.groupby("market_key"):
        ...
        if payout_a is not None and payout_b is not None:
            ...
            if pa is not None and pb is not None and max(pa, pb) >= args.infer_winner_threshold and pa != pb:
                inferred = True
                if pa > pb:
        ...
        pnl_if_a = 100.0 * net_a - cost_total
        pnl_if_b = 100.0 * net_b - cost_total
        min_pnl = min(pnl_if_a, pnl_if_b)
        max_pnl = max(pnl_if_a, pnl_if_b)
        convexity_index = max_pnl / abs(min_pnl) if min_pnl < 0 and abs(min_pnl) > 0 else None
    ...
    report = {
        ...
        "removed_band": f"{band_lo}-{band_hi}",
    }
