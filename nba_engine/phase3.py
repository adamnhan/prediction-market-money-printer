...
logger = logging.getLogger("nba_phase3")
...
class PaperStore:
    ...
    def load_open_positions(self) -> dict[str, Position]:
        rows = self.conn.execute(
            """
            SELECT id, market_ticker, side, entry_ts, entry_price
            FROM paper_trades
...
def _compute_pnl(side: str, entry_price: float, exit_price: float) -> float:
    if side.upper() == "YES":
...
def _exit_signal(
    ...
    pnl = _compute_pnl(position.side, position.entry_price, exit_price)
    if _is_valid_number(price):
        if pnl >= TAKE_PROFIT:
...
def run_loop() -> None:
    ...
    if sample_count == KILL_SWITCH_SAMPLE and mean_pnl is not None and mean_pnl < 0:
        kill_switch = True
        logger.info(
            "kill_switch_triggered mean_pnl=%.4f sample=%d",
            mean_pnl,
            sample_count,
        )

    logger.info(
        "phase3_ready candle_db=%s paper_db=%s last_rowid=%d open_positions=%d %s",
        candle_db_path,
        paper_db_path,
        last_rowid,
        len(positions),
        ...
        for candle in rows:
            ...
            if position:
                ...
                if reason and exit_price is not None and pnl is not None:
                    ...
                    logger.info(
                        "exit %s side=%s reason=%s exit_ts=%s exit_price=%.4f pnl=%.4f entry_ts=%s entry_price=%.4f",
